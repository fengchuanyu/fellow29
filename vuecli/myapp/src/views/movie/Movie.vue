<template>
  <div>
    <ul>
      ...
      >
        <div class="list-left">
          <img :src="getImage(item.cover.url)" alt />
        </div>
...
export default {
  ...
  mounted() {
    window.onscroll = () => {
      ...
      if (
        clientHeight + scrollTop >= scrollHeigth - 10 &&
        this.isLoading &&
        this.dataList.length != 22
      ) {
        this.listStart += 10;
        this.getData();
      }
  ...
  methods: {
    getImage(url) {
      if (url !== undefined) {
        console.log(url.replace(/http\w{0,1}:\/\//g,'https://images.weserv.nl/?url='))
